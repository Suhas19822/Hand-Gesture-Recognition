...
class CTRNN(tf.compat.v1.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.compat.v1.variable_scope(scope or type(self).__name__):
            with tf.compat.v1.variable_scope("RNN",reuse=tf.AUTO_REUSE):  # Reset gate and update gate.
                if(not self.fix_tau):
                    tau = tf.compat.v1.get_variable('tau', [],initializer=tf.constant_initializer(self.tau))
                    self._tau_var = tau
